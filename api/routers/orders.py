...
def list_employee_orders(db: Session = Depends(get_db)):
    return controller.read_all(db)

@employee_router.get("/{order_id}", response_model=schema.Order)
def get_employee_order(order_id: int, db: Session = Depends(get_db)):
